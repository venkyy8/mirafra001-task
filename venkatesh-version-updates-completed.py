...
def update_new_product_code_in_file(vdproj_path, new_product_code):
    try:
        with open(vdproj_path, 'r') as file:
            content = file.read()
            # Regular expression pattern to match the ProductCode line
            product_code_pattern = re.compile(r'"ProductCode" = "8:{(.*?)}"')
            # Replace the existing product code with the new one
            updated_content = product_code_pattern.sub('"ProductCode" = "8:{%s}"' % new_product_code, content)
        
        # Write the updated content back to the file
        with open(vdproj_path, 'w') as file:
            file.write(updated_content)
        
        print("Product code updated successfully.")
    except FileNotFoundError:
        print(f"File '{vdproj_path}' not found.")
    except Exception as e:
        print("An error occurred while updating the product code:", e)
...
def creating_new_product_code(version, version_type):
    # Modify the version string based on the version type
    if version_type == 1:  # Major update
        version += ".0"
    elif version_type == 2:  # Minor update
    ...
    # Concatenate version with a unique identifier (version + timestamp)
    version_string = version + "_" + str(int(time.time())) + "_your_unique_identifier"
    print("Version String:", version_string)  # Debugging statement
    # Hash the concatenated string to generate a product code
    product_code = hashlib.md5(version_string.encode()).hexdigest().upper()
    # Format the product code into GUID style
    formatted_product_code = "{" + product_code[:8] + "-" + product_code[8:12] + "-" + product_code[12:16] + "-" + product_code[16:20] + "-" + product_code[20:] + "}"
    print("Generated Product Code:", formatted_product_code)  # Debugging statement
    return formatted_product_code
            ...
            if user_choice == "all":
                ...
                for choice in user_choice:
                    if choice.isdigit() and 1 <= int(choice) <= len(sub_projects):
                        ...
                        break  # Break out of the loop and ask again

                if len(selected_projects) == len(user_choice):
...
def update_version(assembly_version, properties_version, version_type,base_path,assembly_info_path,vdproj_path,sub_projects_list):
    ...
    if compare_versions(assembly_version, properties_version):
        ...
        time.sleep(1)
        version = extract_properties_version(vdproj_path)
        new_product_code = creating_new_product_code(version, version_type)
        update_new_product_code_in_file(vdproj_path, new_product_code.strip("{}"))
        ...
        if before_updating_MSI_version_Product_code != after_updating_MSI_version_Product_code:
            ...
            if before_updating_MSI_version_upgrade_code != after_updating_MSI_version_upgrade_code:
                ...
                print("Upgrade codes match before and after updating. Continuing further steps.")
        else:
...
def main():
    ...
    # version = extract_properties_version(vdproj_path)
    # product_code = fetch_product_code(vdproj_path)
